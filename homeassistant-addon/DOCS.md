...
Replaces the full tool catalog (~93 tools) with search-based discovery (~4 proxy tools). When enabled, tools are found via `ha_search_tools` and executed through categorized proxies (read/write/delete).
...
- `ha_config_list_view_sections` — List sections of a 'sections'-type dashboard view.
- `ha_config_remove_card` — Remove a card from a dashboard view.
- `ha_config_remove_view` — Remove a dashboard view and all cards within it.
- `ha_config_set_card` — Insert, replace, or move a card within a dashboard view.
...
- `ha_config_set_view` — Create a new dashboard view or update an existing one.
